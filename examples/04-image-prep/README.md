...
`@executioncontrolprotocol/node` and `@executioncontrolprotocol/policies` resolve from npm at `^0.14.0`. For unpublished CLI builds, link the CLI from the core checkout (`pnpm link --global` in `packages/cli` after `pnpm run build`).
