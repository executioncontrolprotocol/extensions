...
function capability(id: string): Handler {
  const cap = adobeExtension.capabilities.find((c) => c.id === id)
...
describe("@executioncontrolprotocol/adobe capabilities", () => {
  beforeEach(async () => {
  ...
  it("registers extension with generated capability count matching OpenAPI", () => {
    ...
    expect(adobeExtension.capabilities).toHaveLength(openApiOps)

    const ids = adobeExtension.capabilities.map((c) => c.id).sort()
    expect(ids).toEqual([...meta.operations].sort())
    expect(globalRegistry.getExtension("@executioncontrolprotocol/adobe")).toBe(adobeExtension)
  })

  it("exposes Zod input/output schemas on every capability", () => {
    for (const cap of adobeExtension.capabilities) {
  ...
  it("sample fixtures safeParse for representative family inputs", () => {
    const fixtures: { id: string; input: unknown }[] = [
      {
        id: "@executioncontrolprotocol/adobe.firefly.generate-images-v3-async",
        input: { body: { prompt: "sunset" } },
      },
      {
        id: "@executioncontrolprotocol/adobe.photoshop.get-job-status",
        input: { path: { jobId: "job-1" } },
      },
      {
        id: "@executioncontrolprotocol/adobe.express.get-job-status",
        input: { path: { jobId: "job-2" } },
      },
      {
        id: "@executioncontrolprotocol/adobe.indesign.list-app-versions",
        input: {},
      },
      {
        id: "@executioncontrolprotocol/adobe.substance3d.create-space-v1",
        input: {},
      },
      {
        id: "@executioncontrolprotocol/adobe.illustrator.facade-job-status",
        input: { path: { jobId: "job-3" } },
      },
      {
        id: "@executioncontrolprotocol/adobe.creative-production.list-batches",
        input: {},
      },
      {
        id: "@executioncontrolprotocol/adobe.audio-video.voices",
        input: {},
      },
    ]
